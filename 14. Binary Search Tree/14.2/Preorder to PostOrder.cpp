...
public:
    ...
    int i = 0;
    Node*build(int pre[] , int UP , int size){
        if(i>=size || pre[i] > UP)return NULL;
        
        
        Node *root =  newNode(pre[i++]);
        root->left = build(pre  , root->data , size);
        root->right = build(pre  , UP , size);
        return root;
    }
    Node* post_order(int pre[], int size)
    {
        //code here
        return build(pre , INT_MAX , size);
    }
};
